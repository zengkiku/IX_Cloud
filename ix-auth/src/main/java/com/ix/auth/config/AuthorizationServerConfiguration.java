    ...
    public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.apply(authorizationServerConfigurer.tokenEndpoint((tokenEndpoint) -> {
            ...
            tokenEndpoint.accessTokenRequestConverter(accessTokenRequestConverter())
                    .accessTokenResponseHandler(new IXAuthenticationSuccessEventHandler()) // 登录成功处理器
                    .errorResponseHandler(new IXAuthenticationFailureEventHandler()); // 登录失败处理器
        }).clientAuthentication(oAuth2ClientAuthenticationConfigurer -> { // 个性化客户端认证
            oAuth2ClientAuthenticationConfigurer.errorResponseHandler(new IXAuthenticationFailureEventHandler());  // 处理客户端认证异常
        }).authorizationEndpoint(authorizationEndpoint -> authorizationEndpoint.consentPage(SecurityConstants.CUSTOM_CONSENT_PAGE_URI))); // 授权码端点个性化confirm页面
        ...
        DefaultSecurityFilterChain securityFilterChain = http.authorizeHttpRequests(authorizeRequests -> {
                ...
                .and()
                .apply(new FormIdentityLoginConfigurer()).and().build(); // 授权码登录的登录页个性化
